...
impl ClaudeWriter {
    ...
    fn write_to_path_inner(
        ...
        let tool_dir = resolve_tool_config_dir("claude", path, dirs.home_dir())
            .ok_or_else(|| AikitError::TargetWrite("unknown claude config directory".into()))?;
        ensure_tool_present_for_new_config("claude", path, &tool_dir)?;
