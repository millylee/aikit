...
- For Claude Code: pin every model slot (main, small/fast, and the Haiku/Sonnet/Opus defaults) to the selected model; the 1M context option appends a `[1m]` model-name suffix and sets `CLAUDE_CODE_AUTO_COMPACT_WINDOW`. For Codex CLI: the 1M context option writes `model_context_window = 1000000` and `model_auto_compact_token_limit = 900000` (and removes them when off). Options default on and can be toggled in the Apply To pane under the Options group.
...
- `Enter` / `Space`: activate the selected provider, API key, model, toggle the selected target, or toggle an option (Pin all Claude models / 1M context / Bypass permissions).
